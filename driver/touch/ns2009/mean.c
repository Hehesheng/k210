...
struct mean_filter_t *mean_alloc(int length)
{
    ...
    for (i = 0; i < length; i++) filter->buffer[i] = 0;
...
void mean_free(struct mean_filter_t *filter)
{
    if (filter)
    {
        if (filter->buffer) touchscreen_free(filter->buffer);
        touchscreen_free(filter);
    }
}
...
int mean_update(struct mean_filter_t *filter, int value)
{
    ...
    if (filter->count < filter->length) filter->count++;
...
void mean_clear(struct mean_filter_t *filter)
{
    ...
    if (filter)
    {
        for (i = 0; i < filter->length; i++) filter->buffer[i] = 0;
        filter->index = 0;
        filter->count = 0;
        filter->sum   = 0;
    }
