...
int ns2009_hal_i2c_init_default(void);
...
int touchscreen_init(void* arg)
{
    ...
    if (ts_ns2009_pdata == NULL || err != 0) return err;
    is_init = true;

    return 0;
}
bool touchscreen_is_init() { return is_init; }
...
int ns2009_hal_i2c_recv(const uint8_t* send_buf, size_t send_buf_len, uint8_t* receive_buf, size_t receive_buf_len)
{
